...
- Global caches used by an installed tool (`~/.npm` with `npm` installed, `~/.cache/uv` with `uv`, …; found in PATH or user-level install directories) are reported as `caution`, so `clean --yes` no longer deletes caches in use. Without the tool they stay `safe`, with a note that no installed tool uses them.
- Homebrew is reclaimed by `brew cleanup` (autoremove disabled) as a scan item sized by brew's dry-run, instead of deleting `~/Library/Caches/Homebrew`. It removes old formula versions under the Homebrew prefix too, and keeps the downloads brew still uses. `brew cleanup` is removed from `--vendor-cleanup`.
- `clean --dry-run` prints the command each command-based item runs; the interactive prompt asks for trash/permanent only when path items are selected.
- The walk engine's no-follow symlink policy is now explicit: it skips any symlink entry (`os.ModeSymlink`) rather than relying on `os.ReadDir`'s incidental `IsDir()==false`, so a future refactor can't silently start following links. Behavior is unchanged — a symlink is never descended into or matched as an artifact (even a symlinked `node_modules` from pnpm/monorepos), which avoids double-counting the target's disk space and never reclaims a shared target; symlink cycles remain unwalkable. Documented as the engine's symlink policy in `docs/architecture.md`.
