...
- Global caches used by an installed tool (`~/.npm` with `npm` in PATH, `~/.cache/uv` with `uv`, …) are reported as `caution`, so `clean --yes` no longer deletes caches in use. Without the tool they stay `safe`, with a note that no installed tool uses them.
- Homebrew is reclaimed by `brew cleanup -s` (autoremove disabled) as a scan item sized by brew's dry-run, instead of deleting `~/Library/Caches/Homebrew`. The size now includes outdated versions under the Homebrew prefix. `brew cleanup` is removed from `--vendor-cleanup`.
