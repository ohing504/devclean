...
- **Node.js**: a pnpm-installed `node_modules` (detected by `node_modules/.modules.yaml`) is annotated that its files are shared with the pnpm store, so deleting it frees space only after `pnpm store prune`.
