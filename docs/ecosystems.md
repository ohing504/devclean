...
AI coding-tool home directories are deliberately **excluded** from the catalog. The whole `~/.claude` tree (session transcripts, project memory, agents, skills, plugins, todos) plus `~/.codex`, `~/.gemini`, and Claude Code's `~/Library/Caches/claude-cli-nodejs` are user state. Deleting any of it is unrecoverable loss, and the "caches" inside only reappear as install-time scaffolding — so reclaiming them is worthless against that risk. devclean never offers them for deletion. (Emulator user data `~/.android/avd` is excluded for the same reason.)
